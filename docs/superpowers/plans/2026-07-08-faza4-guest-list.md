...
- [x] Trimis din setGuestStatus la approved DACĂ toggle-ul org (default ON [D]) ȘI email_notify [C §6.9.1]; conținut: bilete + passes [C]
- [x] Commit

### Task 3: Grid manager (§6.9.3 [C] — comportamentele de tastatură)
- [x] Rând "New Guest" permanent sus; Tab/Shift-Tab între câmpuri; Enter/Shift-Enter între rânduri; în Notes Enter = rând nou de text, Tab iese; Tab din Notes pe rândul nou finalizează guestul și mută focus pe Last Name
- [x] Coloane [C-S]: LAST|FIRST|#TIX|câte o coloană per pass type|STATUS|AFFILIATION|REQUESTOR|DATE|PICKUP|PRIORITY (Space comută)|NOTES|EMAIL NOTIFY
- [x] Smart defaults (valorile devin default pe New Guest) [C]; filtre Requestor/Status/Affiliation; master checkbox pe selecția filtrată; bulk status; sortare pe coloane; totaluri DOAR pe selecția filtrată [C]
- [x] Header: REQUEST CUTOFF + toggle LOCKED [C-S]; footer per coloană numerică: NUM ALLOWED + ENFORCED checkbox + Remaining ROȘU la negativ [C]
- [x] Panou detaliu cu buton APPROVE [C-S]
- [x] Commit

### Task 4: Form crew + setări
- [x] Form simplu (gl_submit): nume, affiliation, nr bilete, pass type, note → Pending [C §6.9.4]; vede doar requesturile proprii + status
- [x] Tour passes CRUD în setările turului; toggle emailuri în org settings [C]
- [x] Commit

### Task 5: Verificare
- [x] Edge cases [C §6.9.3]: UN singur pass type nu dă eroare la submit; mesaj cutoff corect după expirare — în teste
- [x] lint/typecheck/test/build + test-rls.sh verzi
